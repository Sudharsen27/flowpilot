"""Lead qualification analysis records.

Revision ID: 009_lead_qualifications
Revises: 008_leads
Create Date: 2026-09-10

Stores AI understanding results separately from CRM Lead rows. Does not
change Lead.status. Result JSON is a validated analysis object, not a raw
provider payload.
"""
...
revision: str = "009_lead_qualifications"
down_revision: str | None = "008_leads"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("organization_id", "id", name="uq_lead_qualifications_organization_id"),
    )
    op.create_index(
        op.f("ix_lead_qualifications_organization_id"),
        "lead_qualifications",
        ["organization_id"],
    )
    op.create_index(
        op.f("ix_lead_qualifications_lead_id"),
        "lead_qualifications",
        ["lead_id"],
    )
    op.create_index(
        op.f("ix_lead_qualifications_initiated_by_user_id"),
        "lead_qualifications",
